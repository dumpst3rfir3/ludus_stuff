...
* **[basic-ad-config.yml](basic-ad-config.yml):** This is my config for a very
  basic Active Directory setup that simulates a small company with presence in
  the US and CA. It sets up:
    - A domain: dumpster.fire 
    - A DC running Windows Server 2022
    - A separate Windows 2022 server (non-DC)
        * If the
          [Add-ExtraConfigForBasicAD.ps1](scripts/Add-ExtraConfigForBasicAD.ps1)
          script in the scripts folder is used, this server will also be a file
          server hosting SMB shares
...
> See the [Ludus documentation](https://docs.ludus.cloud/docs/roles) for more
> info. 

* **[basic-ad-with-attackers-config.yml](basic-ad-with-attackers.yml):** This is
  the same as `basic-ad-config.yml` above, only it adds two Kali machines:
